...
public class Cmd_battle_spawn_setspawn {
	...
	public boolean onCmdSetspawn(String[] args, Player player) {
		...
		plugin.getGame().getSpawn().getLocation().getWorld().setSpawnLocation((int) plugin.getGame().getSpawn().getLocation().getX(), (int) plugin.getGame().getSpawn().getLocation().getY(), (int) plugin.getGame().getSpawn().getLocation().getZ());
