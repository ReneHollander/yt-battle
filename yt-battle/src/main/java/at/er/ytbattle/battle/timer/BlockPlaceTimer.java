	...
	public void run() {
		if (time == 0) {
			for (String s : t.getPlayers()) {
				...
				p.sendMessage(Battle.prefix() + "Place a wool to disable this effect");
		...
		if (time == this.initTime) {
			for (String s : t.getPlayers()) {
				...
				p.sendMessage(Battle.prefix() + "You have " + time / 60 + " minutes left to place a wool.");
			}
		}
		...
		if (time % 10 == 0 && time > 0 && time < 60) {
			for (String s : t.getPlayers()) {
				...
				p.sendMessage(Battle.prefix() + "You have " + time + " seconds left to place a wool.");
			}
		}
