...
public class BorderCreator {
	...
	public String build(int radius) {
		...
		try {
			hcb.build(new EditSession(new BukkitWorld(plugin.getGame().getSpawn().getLocation().getWorld()), 2147483647), 
			new Vector(plugin.getGame().getSpawn().getLocation().getX(), 0, plugin.getGame().getSpawn().getLocation().getZ()), 
			new SingleBlockPattern(new BaseBlock(20, 0)), 
			radius);
