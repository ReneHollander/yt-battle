...
public class Cmd_battle_help_reset {
	...
	public boolean onCmdHelp(String[] args, Player player) {
		player.sendMessage(Battle.prefix()
				+ "Battle Commands:  - <> = has to be attached [] = can be attached \n"
				...
				+ " - /battle border <borderradius> - Creates a Glassborder with attached radius. WorldEdit has to be installed!"
				+ " - /battle reset - Resets the Battle\n");
		return true;
	}
	
	public boolean onCmdReset(String[] args, Player player) {
		/*boolean old = plugin.getConfig().getBoolean("config.enable-automatic-load");
