...
public class BlockBreakListener implements Listener {
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (Battle.instance().getGame().getSpawn() != null) {
			if (event.getBlock().getType() == Material.GLASS && player.getWorld() == Battle.instance().getGame().getSpawn().getLocation().getWorld() && Battle.instance().getGame().isStarted() && player.getGameMode() != GameMode.CREATIVE) {
				event.setCancelled(true);
